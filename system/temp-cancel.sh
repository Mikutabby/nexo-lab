...
echo "$SUDO_PASS" | sudo -S systemctl cancel 2>/dev/null
echo "$SUDO_PASS" | sudo -S rtcwake -m disable 2>/dev/null
echo "$SUDO_PASS" | sudo -S sh -c 'echo 0 > /sys/class/rtc/rtc0/wakealarm' 2>/dev/null
