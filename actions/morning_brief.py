"""
morning_brief.py — Informe matutino inteligente de JARVIS.
...
def morning_brief(parameters: dict = None, player=None, **kwargs) -> str:
    """
    Genera el informe matutino completo de JARVIS.
    ...
    result = " ".join(parts)
    if player:
        player.write_log(f"JARVIS: {result[:120]}…")
    return result


def brief() -> str:
    """Genera el informe matutino (wrapper simple)."""
    return morning_brief(parameters={"force": True})
