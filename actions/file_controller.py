...
# Rutas absolutamente prohibidas (sistema operativo crítico)
_FORBIDDEN = [
    Path("C:/Windows"), Path("C:/Windows/System32"),
    Path("/usr/bin"), Path("/bin"), Path("/sbin"), Path("/etc"),
    Path("/System"), Path("/Library/System"),
]

def _is_safe_path(target: Path) -> bool:
    """Bloquea solo rutas de sistema crítico; permite todo lo demás."""
    try:
        ...
        return True  # si no se puede resolver, dejar pasar y que el OS decida
...
def _get_desktop() -> Path:
    if _OS == "Windows":
        ...
        if xdg and Path(xdg).exists():
            return Path(xdg)
        # Probar nombres comunes en diferentes idiomas
        for name in ["Desktop", "Escritorio", "Área de Trabalho"]:
            p = Path.home() / name
            if p.exists():
                return p
...
def _get_videos() -> Path:
    if _OS == "Linux":
        ...
        if xdg and Path(xdg).exists():
            return Path(xdg)
        for name in ["Videos", "Vídeos", "Videos"]:
            p = Path.home() / name
            if p.exists():
                return p
...
def create_file(path: str, name: str = "", content: str = "") -> str:
    try:
        ...
        if _HAS_FE:
            _fe.emit_code(abs_path, content)
...
def create_folder(path: str, name: str = "") -> str:
    try:
        ...
        return f"✅ Carpeta creada: {target.resolve()}"
    except Exception as e:
...
def edit_file(path: str, name: str = "", old_text: str = "",
              new_text: str = "", mode: str = "replace") -> str:
    """
    Edita un archivo de texto existente.
    mode: replace (reemplaza old_text por new_text) |
          append (agrega al final) |
          prepend (agrega al inicio) |
          overwrite (reescribe todo con new_text)
    """
    try:
        base   = _resolve_path(path)
        target = (base / name) if name else base
        ...
        if mode == "replace":
            if not old_text:
                return "Especificá 'old_text' para reemplazar."
            if old_text not in content:
                return f"Texto no encontrado en {target.name}: '{old_text[:60]}'"
            ...
            return f"Modo desconocido: '{mode}'. Opciones: replace, append, prepend, overwrite."

        target.write_text(content, encoding="utf-8")
        return f"✅ Archivo editado: {target.name}"
    except Exception as e:
    ...
    try:
        if action == "list":
            ...
            if not params.get("confirm"):
                return (f"⚠️ ¿Confirmás que querés eliminar '{target.name}'? "
                        f"Esta acción moverá el archivo a la papelera. "
                        f"Respondé 'sí, eliminalo' para confirmar.")
