"""
knowledge_base.py — Segundo cerebro / base de conocimiento personal para JARVIS.
...
def knowledge_base(parameters: dict = None, response=None, player=None, session_memory=None) -> str:
    ...
    # ── export ────────────────────────────────────────────────────────────────
        ...
        if not entries:
            return "La base de conocimiento está vacía."
        if not out_path:
            out_path = str(Path.home() / "jarvis_knowledge_export.md")

        lines = ["# JARVIS — Base de Conocimiento\n"]
        for e in entries:
            icon = _TYPE_ICONS.get(e.get("type","note"), "📝")
            tags_str = f" `{'` `'.join(e['tags'])}`" if e.get("tags") else ""
...
def kb_add(title: str, content: str, kind: str = "note", tags: list | None = None) -> str:
...
def kb_search(query: str) -> str:
    ...
    return knowledge_base({"action": "search", "query": query})


def kb_list() -> str:
    """Lista entradas de la base de conocimiento."""
    return knowledge_base({"action": "list"})
