"""
spotify_control.py — Control total de Spotify para JARVIS.
...
def _send_spotify_widget(sp, player, is_playing: bool = True):
    """Fetch current playback and send widget update to the JARVIS UI."""
    if not player:
        return
    try:
...
def spotify(action: str = "current", query: str = "") -> str:
    """
    Controla Spotify con interfaz simple.
    
    Args:
        action: play, pause, next, previous, current, search
        query: Término de búsqueda (solo para search/play)
    ...
    return spotify_control(parameters=params, player=None)