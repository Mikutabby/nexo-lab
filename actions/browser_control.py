...
class _BrowserSession:
    ...
    async def _launch_chrome_cdp(self, exe: str):
        ...
        Uses a JARVIS-dedicated user-data-dir to avoid profile lock conflicts.
        """
        # 1. Port already open → try connecting (may be a previous JARVIS Chrome)
        ...
        jarvis_dir = str(Path.home() / ".jarvis_profiles" / "chrome_cdp")
    ...
    async def _launch(self):
        ...
        # Chrome on Windows → use CDP with a JARVIS-dedicated profile
...
def browser_control(
    ...
    # Per-action timeouts (seconds) — keeps JARVIS responsive
